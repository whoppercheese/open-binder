...
type AddToChecklistSheetProps = {
  cardId: string;
  open: boolean;
  onClose: () => void;
  onSaved?: (checklistCount: number) => void;
};
...
export function AddToChecklistSheet({
  cardId,
  open,
  onClose,
  onSaved,
}: AddToChecklistSheetProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(t("sets.checklistLoadFailed"));
        setOptions([]);
        return;
      }
      const items: ChecklistOption[] = payload.collections ?? [];
      setOptions(items);
  ...
  const checklistCount = alreadyOnChecklist.length;
  ...
  async function fetchChecklistCount(): Promise<number> {
    ...
    const items: ChecklistOption[] = payload.collections ?? [];
    return items.filter((item) => item.onChecklist).length;
  }
